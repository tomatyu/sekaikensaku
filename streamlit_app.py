...
if any(countries_df["国名"] == a):
    ...
    st.write("検索結果なし")selected_country = countries_df[countries_df["国名"] == a].iloc[0]
usa_df = pd.DataFrame({
...
if any(countries_df["国名"] == a):
    ...
    stselected_country = countries_df[countries_df["国名"] == a].iloc[0]
usa_df = pd.DataFrame({
    'lat': [(selected_country["緯度"])],  # 緯度
    'lon': [(selected_country["経度"])]  # 経度
   }).write("国名:", selected_country["国名"])
    st.write("首都:", selected_country["首都"])
